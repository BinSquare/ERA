...
```bash
curl -X POST https://anewera.dev/api/sessions/node-app/run \
  -H "Content-Type: application/json" \
  -d '{
...
curl -X POST https://anewera.dev/api/sessions/deno-app/run \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
response=$(curl -s -X POST "https://anewera.dev/api/sessions" \
  -H "Content-Type: application/json" \
  -d '{
    "language": "python",
...
```bash
# Check setup status
curl -s "https://anewera.dev/api/sessions/my-session" | jq '{setup_status, setup_result}'
```
...
curl -X POST https://anewera.dev/api/sessions/prod-analytics/run \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl -s "https://anewera.dev/api/sessions/my-session" | jq '.setup_result'
```
